...
function saveFriends() {
	fs.writeFile(FRIENDS_PATH, JSON.stringify(friendsData, null, 2), (err) => {
...
if (fs.existsSync(PLAYERS_PATH)) {
	try {
		playersData = JSON.parse(fs.readFileSync(PLAYERS_PATH, "utf8"));
	} catch (e) {}
}

function savePlayers() {
	fs.writeFile(PLAYERS_PATH, JSON.stringify(playersData, null, 2), (err) => {
		if (err) console.error(err);
	});
}
...
function updateRealTimeWeather() {
	...
	const url =
		"https://api.open-meteo.com/v1/forecast?latitude=52.0907&longitude=5.1214&current=is_day,weather_code&daily=sunrise,sunset&timezone=GMT";
...
function createRoom(id, options = {}) {
	...
	rooms[id] = {
		...
		settings: {
			...
			allowBuilding: options.allowBuilding || false,
			noSmoking: options.noSmoking || false,
			isOutside: options.isOutside || false,
	...
	rooms[id].items.forEach((i) => {
		if (!i.id) i.id = Date.now().toString(36) + Math.random().toString(36).substr(2) + Math.random().toString(36).substr(2);
	});
...
io.on("connection", (socket) => {
	...
	socket.on("joinRoom", (data) => {
		...
		if (!playersData[userId]) {
			playersData[userId] = { wallet: 0, inventory: [], debt: 0 };
		}
		...
		if (!room) {
			room = createRoom(roomId, {
				...
				allowBuilding: data.allowBuilding === true,
				noSmoking: data.noSmoking === true,
				isOutside: data.isOutside === true,
				weather: data.weather || "clear",
				time: data.time || "day",
			});

			// NIEUW: Apply Debt if creating a studio (check via room name convention or just size logic)
			if (roomId.includes("-studio")) {
				const size = data.size || 10;
				const cost = STUDIO_COSTS[size] || 1000;
				if (!playersData[userId].debt) playersData[userId].debt = 0;
				playersData[userId].debt += cost;
				savePlayers();
				console.log(`Studio created for ${userId}. Added debt: ${cost}. Total: ${playersData[userId].debt}`);
			}
			...
			let changed = false;
			if (data.alwaysOnline !== null && data.alwaysOnline !== undefined) {
				room.settings.alwaysOnline = data.alwaysOnline;
				changed = true;
			}
			if (data.allowBuilding !== null && data.allowBuilding !== undefined) {
				room.settings.allowBuilding = data.allowBuilding;
				changed = true;
			}
			if (data.noSmoking !== null && data.noSmoking !== undefined) {
				room.settings.noSmoking = data.noSmoking;
				changed = true;
			}
		...
		socket.emit("initGame", {
			...
			debt: playersData[userId].debt || 0, // NIEUW: Stuur schuld mee
		});
	...
	socket.on("placeItem", (item) => {
		...
		// Determine Ownership
		if (room.ownerId) {
			item.ownerId = socket.userId; // Private room: owner is dropper
		} else {
			item.ownerId = null; // Public room: no owner
		}

		item.lastTouchedBy = socket.id; // Voor physics updates
			...
			fs.writeFile(imagePath, data.imageData, (err) => {
				if (!err) {
					...
					if (obj.image && obj.image.startsWith("/uploads/")) {
						try {
							fs.unlinkSync(path.join(__dirname, obj.image));
						} catch (e) {}
					}
					obj.image = imageUrl;
					finishUpdate();
