...
let myRoomSettings = { doorbell: false, alwaysOnline: false, allowBuilding: false, noSmoking: false, isOutside: false, weather: 'clear', time: 'day' }; // Instellingen van MIJN kamer
let currentRoomSettings = { doorbell: false, alwaysOnline: false, allowBuilding: false, noSmoking: false, isOutside: false, weather: 'clear', time: 'day' }; // Instellingen van huidige kamer
...
try {
    ...
    socket.on('connect', () => {
        ...
        socket.emit('joinRoom', { roomId: roomId, ...savedPos, size: sizeToSend, userId: myUserId, alwaysOnline: alwaysOnlineParam, allowBuilding: allowBuildingParam, noSmoking: noSmokingParam, isOutside: isOutsideParam, weather: weatherParam || 'clear', time: timeParam || 'day' });
    ...
    socket.on('myRoomData', (data) => {
        if (data.roomId) {
            ...
            myRoomSettings = data.settings || { doorbell: false, alwaysOnline: false, allowBuilding: false, noSmoking: false, isOutside: false, weather: 'clear', time: 'day' };
    ...
    socket.on('initGame', (data) => {
        ...
        currentRoomSettings = data.roomSettings || { doorbell: false, alwaysOnline: false, allowBuilding: false, noSmoking: false, isOutside: false, weather: 'clear', time: 'day' };
...
if (createNewProjectConfirmBtn) {
    createNewProjectConfirmBtn.addEventListener('click', () => {
        ...
        localStorage.setItem('habboCloneLastProjectSettings', JSON.stringify({
            width: width,
            height: height,
        ...
        projectCanvasWrapper.style.background = 'conic-gradient(#ccc 0.25turn, white 0.25turn 0.5turn, #ccc 0.5turn 0.75turn, white 0.75turn)';
        ...
        guideV.id = 'guideV'; guideV.className = 'project-guide-line vertical';
        const guideH = document.createElement('div');
        guideH.id = 'guideH'; guideH.className = 'project-guide-line horizontal';
...
if (projectWorkspace) {
    ...
    projectWorkspace.addEventListener('mousedown', (e) => {
        if (currentProjectTool === 'hand' && (e.button === 0 || e.button === 1)) {
            ...
            if (selectionRect && 
                lx >= selectionRect.x && lx <= selectionRect.x + selectionRect.w &&
                ly >= selectionRect.y && ly <= selectionRect.y + selectionRect.h) {
...
function updateProjectSubHeader() {
    ...
    if (currentProjectTool === 'brush') {
        createControl('Type:', createSelect(['Rond', 'Vierkant'], projectToolSettings.brush.type === 'round' ? 'Rond' : 'Vierkant', e => projectToolSettings.brush.type = e.target.value === 'Rond' ? 'round' : 'square'));
        createControl('Grootte:', createInput('number', projectToolSettings.brush.size, 1, 100, e => projectToolSettings.brush.size = parseInt(e.target.value), '40px'));
        
        const opWrapper = document.createElement('div');
        opWrapper.style.display = 'flex';
        opWrapper.style.alignItems = 'center';
        opWrapper.appendChild(createInput('number', projectToolSettings.brush.opacity, 0, 100, e => projectToolSettings.brush.opacity = parseInt(e.target.value), '40px'));
        ...
        createControl('Type:', createSelect(['Zacht', 'Hard'], projectToolSettings.eraser.type === 'soft' ? 'Zacht' : 'Hard', e => projectToolSettings.eraser.type = e.target.value === 'Zacht' ? 'soft' : 'hard'));
        createControl('Grootte:', createInput('number', projectToolSettings.eraser.size, 1, 100, e => projectToolSettings.eraser.size = parseInt(e.target.value), '40px'));
        
        const opWrapper = document.createElement('div');
        opWrapper.style.display = 'flex';
        opWrapper.style.alignItems = 'center';
        opWrapper.appendChild(createInput('number', projectToolSettings.eraser.opacity, 0, 100, e => projectToolSettings.eraser.opacity = parseInt(e.target.value), '40px'));
        const span = document.createElement('span');
        span.innerText = '%';
        span.style.marginLeft = '2px';
        opWrapper.appendChild(span);
        ...
        createControl('Font:', createSelect(['Arial', 'Verdana', 'Times New Roman', 'Courier New'], projectToolSettings.text.font, e => projectToolSettings.text.font = e.target.value));
        createControl('Stijl:', createSelect(['Regular', 'Bold', 'Italic', 'Bold Italic'], projectToolSettings.text.family, e => projectToolSettings.text.family = e.target.value));
        createControl('Pt:', createInput('number', projectToolSettings.text.size, 1, 200, e => projectToolSettings.text.size = parseInt(e.target.value), '40px'));
        ...
        createControl('Uitlijning:', createSelect(['Links', 'Gecentreerd', 'Rechts'], projectToolSettings.text.align, e => projectToolSettings.text.align = e.target.value));
    }
}
...
function createInput(type, value, min, max, onChange, width) {
    ...
    if(type === 'range') inp.oninput = onChange; else inp.onchange = onChange;
    return inp;
}
...
function initRotateDrag(e) {
    ...
    transformStart = {
        ...
        cx: anchorScreenX, cy: anchorScreenY
    };
}
...
window.addEventListener('mousemove', (e) => {
    if (isTransforming && transformMode === 'resize' && activeTransformHandle && transformStart) {
        ...
        if (e.altKey) {
            ...
            if (e.shiftKey) {
                if (activeTransformHandle.length === 2) { // Hoek
                    if (newW / newH > ratio) {
                        newH = newW / ratio;
        ...
        if (newW < 1) { newW = 1; if(activeTransformHandle.includes('w')) newX = anchorX - 1; }
        if (newH < 1) { newH = 1; if(activeTransformHandle.includes('n')) newY = anchorY - 1; }
...
const buildableObjects = [
    ...
    { name: "Kraan", height: 1, image: kraanImg.src, category: 'objecten', subCategory: 'objects', placement: 'floor', isTemplate: true, price: 50, interactionType: 'tap' },
    { name: "Hoge Kraan", height: 2, image: kraanImg96.src, category: 'objecten', subCategory: 'objects', placement: 'floor', isTemplate: true, price: 75, interactionType: 'tap' },
    ...
    { name: "Muur", image: "icons/wall_build.png", category: 'objecten', subCategory: 'wall', placement: 'wall_structure', isTemplate: false, price: 50, wallHeight: 150, wallThickness: 0.25 },
    { name: "Muur", image: "icons/wall_build.png", category: 'objecten', subCategory: 'wall', placement: 'wall_structure', isTemplate: true, price: 50, wallHeight: 150, wallThickness: 0.25 },
    { name: "Muur met Poort (3x)", image: "icons/portier_build.png", category: 'objecten', subCategory: 'wall', placement: 'wall_structure', isTemplate: false, price: 150, wallHeight: 150, wallThickness: 0.25, width: 3 },
    { name: "Muur met Poort (3x)", image: "icons/portier_build.png", category: 'objecten', subCategory: 'wall', placement: 'wall_structure', isTemplate: true, price: 150, wallHeight: 150, wallThickness: 0.25, width: 3 },
    ...
    while(openSet.length > 0){
        ...
        for(let neighbor of neighbors){
            ...
            if (Math.abs(dx) + Math.abs(dy) === 1) {
                ...
                if (isWallBetween(current, c1) || isWallBetween(c1, neighbor) ||
                    isWallBetween(current, c2) || isWallBetween(c2, neighbor)) {
                    wallBlocked = true;
                }
            }
...
function drawBall() {
    drawCharacter(ball.x, ball.y, true, myColor, 0, isSmoking, ball.smokingStartTime, ball.smokingItemType, isDrinking, ball.drinkingStartTime, ball.drinkingItemType);
}
...
window.addEventListener("keydown", (e) => {
    ...
    if (isBuildMode && (activeBuildCategory === 'objecten' || activeBuildCategory === 'wallbuild') && (selectedBuildObject || movingObject || buildTool === 'place_wall')) {
...
window.addEventListener('mousemove', (e) => {
    ...
    if (isBuildMode && (buildTool === 'move' || buildTool === 'delete') && !movingObject) {
        ...
        if (tileKey !== lastHoveredTileKey) {
            ...
            if (currentObjects.length + currentItems.length + currentWallObjs.length + currentWallObjectsOnHover.length !== hoveredObjects.length) {
    ...
    } else if (isBuildMode && (selectedBuildObject || movingObject) && !(selectedBuildObject && selectedBuildObject.placement === 'wall_structure')) {
        ...
        const mWorldX = (e.clientX - (window.innerWidth / 2 + camX)) / scale;
        const mWorldY = (e.clientY - (window.innerHeight / 4 + camY)) / scale;
        hoveredCustomWallForColor = objects.slice().reverse().find(o => o.isWall && isMouseOverWall(o, mWorldX, mWorldY));
...
canvas.addEventListener("mousemove", e=>{
    ...
    if (isMarkerMode && !isBuildMode && !isCameraDragging && !isObjectDragging && !isItemDragging) {
        ...
        let targetWall = objects.slice().reverse().find(o => 
            o.isWall && 
            isMouseOverWall(o, mWorldX, mWorldY)
        );
        ...
        if (!targetWall) {
            ...
            for (let x = 0; x < mapW; x++) {
                ...
                if (isMouseOverWall(wallObj, mWorldX, mWorldY)) { targetWall = wallObj; break; }
            }
        }
        if (!targetWall) {
        ...
        if (targetWall) {
            ...
            if (e.buttons === 1) {
                ...
                if (targetWall.flipped) { // Y-axis muur (vlak op X)
...
canvas.addEventListener("mousedown", e => {
    ...
    if (hasActiveMarker && !isBuildMode && e.button === 0) {
        ...
        let targetWall = objects.slice().reverse().find(o => o.isWall && isMouseOverWall(o, mWorldX, mWorldY));
        if (!targetWall) {
            for (let x = 0; x < mapW; x++) {
                const wallObj = { isWall: true, x: x, y: 0, flipped: false, wallHeight: globalWallHeight, wallThickness: globalWallThickness };
                if (isMouseOverWall(wallObj, mWorldX, mWorldY)) { targetWall = wallObj; break; }
        ...
        if (targetWall) {
            ...
            if (targetWall.flipped) {
                ...
                const yOnWall = wx - mWorldX / 32;
                const zOnWall = (wx + yOnWall) * 16 - mWorldY;
                wallX = wx; wallY = yOnWall; wallZ = zOnWall;
            } else { 
                const wy = targetWall.y;
                const xOnWall = mWorldX / 32 + wy;
                const zOnWall = (xOnWall + wy) * 16 - mWorldY;
                wallX = xOnWall; wallY = wy; wallZ = zOnWall;
            }
            
            if (markerTool === 'draw') {
                const markId = Date.now() + '-' + Math.random().toString(36).substr(2, 9);
                const mark = { x: wallX, y: wallY, z: wallZ, life: 1.0, decayRate: 0, sortOffset: 0.05, color: markerColor, size: markerSize, id: markId };
...
window.addEventListener("mouseup", e => {    
    ...
    if (e.button === 0 && isBuildMode && Math.sqrt(dx*dx + dy*dy) < 5) { // Alleen uitvoeren bij linkermuisknop
        ...
        if ((activeBuildCategory === 'objecten' || activeBuildCategory === 'wallbuild') && ((x >= 0 && y >= 0) || hoverTarget?.type === 'wall')) {
            ...
            if ((buildTool === 'place_wall' || (selectedBuildObject && selectedBuildObject.placement === 'wall_structure')) && allSegmentsValid) {
            ...
            const mWorldX = (e.clientX - (window.innerWidth / 2 + camX)) / scale;
            const mWorldY = (e.clientY - (window.innerHeight / 4 + camY)) / scale;
            const customWall = objects.slice().reverse().find(o => o.isWall && isMouseOverWall(o, mWorldX, mWorldY));
        ...
        const checkCol = (tx, ty) => {
            ...
            return objects.some(o => {
                ...
                if (o.isWall) {
                    ...
                    if (o.flipped) { // Y-axis muur (loopt langs Y)
                        // Staat op X. Loopt van Y tot Y+w. Dikte is X-t tot X.
                        minX = o.x - t;
                        maxX = o.x;
                        minY = o.y;
                        maxY = o.y + w;
                    } else { // X-axis muur (loopt langs X)
                    ...
                    if (tx + radius > minX && tx - radius < maxX && 
                        ty + radius > minY && ty - radius < maxY) {
                ...
                if (tx + radius > o.x && tx - radius < o.x + w && 
                    ty + radius > o.y && ty - radius < o.y + d) {
    ...
    if (isObjectDragging && e.button === 0) { // Alleen voor linkermuisknop
        ...
        for(let dx = 0; dx < w; dx++) {
            for(let dy = 0; dy < d; dy++) {
                const tx = x + dx;
                const ty = y + dy;
                if (tx >= mapW || ty >= mapH || isBlocked(tx, ty, true) || (Math.floor(ball.x) === tx && Math.floor(ball.y) === ty)) isValidPlacement = false;
            }
        }
...
function gameLogic() {
    ...
    if (isObjectDragging) {
        ...
        if (camOriginalPos) { camOriginalPos.x = camX; camOriginalPos.y = camY; }
    }
}
...
function updateAndDrawWeather(ctx, width, height) {
    ...
    if (curTimeOverlay.a > 0.005) {
        ctx.fillStyle = `rgba(${Math.round(curTimeOverlay.r)}, ${Math.round(curTimeOverlay.g)}, ${Math.round(curTimeOverlay.b)}, ${curTimeOverlay.a})`;
        ctx.fillRect(0, 0, width, height);
    }
    if (curWeatherOverlay.a > 0.005) {
        ctx.fillStyle = `rgba(${Math.round(curWeatherOverlay.r)}, ${Math.round(curWeatherOverlay.g)}, ${Math.round(curWeatherOverlay.b)}, ${curWeatherOverlay.a})`;
        ctx.fillRect(0, 0, width, height);
    }
...
function renderLoop() {
    ...
    const objForOutline = (selectedBuildObject?.placement === 'floor' && selectedBuildObject) || movingObject || draggedObject || draggedItem;
    if (objForOutline && hoverCell && !objForOutline.isWall && !objForOutline.wallId) {
        ...
        for(let dx = 0; dx < w; dx++) {
            for(let dy = 0; dy < d; dy++) {
                ...
                } else if (!objForOutline.isFloor && !objForOutline.isItem && objForOutline !== draggedItem && (isBlocked(tx, ty, true) || (Math.floor(ball.x) === tx && Math.floor(ball.y) === ty))) {
                    isValidPlacement = false;
                }
            }
        }
        ...
        if (obj.isWall) {
            ...
            renderList.push({
                ...
                draw: () => {
                    ...
                    ctx.fillStyle = color;
                    ctx.beginPath();
                    ctx.moveTo(c2.sx, c2.sy - h); ctx.lineTo(c3.sx, c3.sy - h); ctx.lineTo(c3.sx, c3.sy - bottomOffset); ctx.lineTo(c2.sx, c2.sy - bottomOffset); ctx.fill();
                    ...
                    ctx.fillStyle = color;
                    ctx.beginPath();
                    ctx.moveTo(c3.sx, c3.sy - h); ctx.lineTo(c4.sx, c4.sy - h); ctx.lineTo(c4.sx, c4.sy - bottomOffset); ctx.lineTo(c3.sx, c3.sy - bottomOffset); ctx.fill();
                    ...
                    if (obj.isGate) {
                        ctx.fillStyle = color;
                        ctx.beginPath();
                        ctx.moveTo(c1.sx, c1.sy - bottomOffset); ctx.lineTo(c2.sx, c2.sy - bottomOffset); ctx.lineTo(c3.sx, c3.sy - bottomOffset); ctx.lineTo(c4.sx, c4.sy - bottomOffset);
                    ...
                    if (highlightOverlay) {
                        ...
                        // Top
                        ctx.beginPath(); ctx.moveTo(c1.sx, c1.sy - h); ctx.lineTo(c2.sx, c2.sy - h); ctx.lineTo(c3.sx, c3.sy - h); ctx.lineTo(c4.sx, c4.sy - h); ctx.closePath(); ctx.fill();
                        // East
                        ctx.beginPath(); ctx.moveTo(c2.sx, c2.sy - h); ctx.lineTo(c3.sx, c3.sy - h); ctx.lineTo(c3.sx, c3.sy - bottomOffset); ctx.lineTo(c2.sx, c2.sy - bottomOffset); ctx.fill();
                        // South
                        ctx.beginPath(); ctx.moveTo(c3.sx, c3.sy - h); ctx.lineTo(c4.sx, c4.sy - h); ctx.lineTo(c4.sx, c4.sy - bottomOffset); ctx.lineTo(c3.sx, c3.sy - bottomOffset); ctx.fill();
                    }
                        ...
                        if (isBuildMode) {
                            ...
                            if ((buildTool === 'move' || buildTool === 'delete') && !movingObject) {
                                if (hoveredObjects.length > 0 && hoveredObjects[moveSelectionIndex] === obj) {
                                 ...
                                 if (hoverCell.x >= obj.x && hoverCell.x < obj.x + hw && 
                                     hoverCell.y >= obj.y && hoverCell.y < obj.y + hd) {
    ...
    const objectToPreview = ((selectedBuildObject?.placement === 'floor' || selectedBuildObject?.placement === 'wall_structure') && selectedBuildObject) || movingObject || draggedObject || (buildTool === 'place_wall' ? { isWallPreview: true } : null);
        ...
        if (objectToPreview.isWallPreview || objectToPreview.isWall || objectToPreview.placement === 'wall_structure') {
             ...
             renderList.push({
                    ...
                    draw: () => {
                        ...
                        const isValid = isFlipped 
                            ? (wx > 0 && wx <= mapW && wy >= 0 && wy < mapH)
                            : (wx >= 0 && wx < mapW && wy > 0 && wy <= mapH);
                        ...
                        ctx.fillStyle = color;
                        ctx.beginPath();
                        ctx.moveTo(c1.sx, c1.sy - h); ctx.lineTo(c2.sx, c2.sy - h); ctx.lineTo(c3.sx, c3.sy - h); ctx.lineTo(c4.sx, c4.sy - h);
                        ctx.closePath(); ctx.fill();
                        ...
                        ctx.fillStyle = color;
                        ctx.beginPath();
                        ctx.moveTo(c2.sx, c2.sy - h); ctx.lineTo(c3.sx, c3.sy - h); ctx.lineTo(c3.sx, c3.sy); ctx.lineTo(c2.sx, c2.sy); ctx.fill();
                        ...
                        ctx.fillStyle = color;
                        ctx.beginPath();
                        ctx.moveTo(c3.sx, c3.sy - h); ctx.lineTo(c4.sx, c4.sy - h); ctx.lineTo(c4.sx, c4.sy); ctx.lineTo(c3.sx, c3.sy); ctx.fill();
            ...
            for(let dy = 0; dy < d; dy++) {
                ...
                if (tx >= mapW || ty >= mapH) {
                    isValidPlacement = false;
                } else if (!objectToPreview.isFloor && !objectToPreview.isItem && (isBlocked(tx, ty, true) || (Math.floor(ball.x) === tx && Math.floor(ball.y) === ty))) {
                    isValidPlacement = false;
                }
            }
            }
